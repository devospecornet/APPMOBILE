...
public class AjouterFicheActivity extends AppCompatActivity {
    ...
    private void enregistrerFiche() {
        ...
        if (mois.isEmpty()) { Toast.makeText(this, R.string.mois_obligatoire, Toast.LENGTH_SHORT).show(); return; }
        if (!MOIS_PATTERN.matcher(mois).matches()) { Toast.makeText(this, R.string.mois_format_invalide, Toast.LENGTH_LONG).show(); return; }
        ...
        try {
            ...
            body.put("taux_tva_essence", 20);
            body.put("taux_tva_petit_dejeuner", 10);
            body.put("taux_tva_repas_midi", 10);
            body.put("taux_tva_repas_soir", 10);
            body.put("taux_tva_hotel", 10);
            ...
            JsonObjectRequest request = new JsonObjectRequest(Request.Method.POST, ApiConfig.BASE_URL + "fiches.php", body,
                    ...
                    error -> {
                        ...
                        Toast.makeText(this, ApiErrorHelper.extractMessage(error, getString(R.string.erreur_creation_fiche)), Toast.LENGTH_LONG).show();
                    }) {
                @Override
