...
public class DetailFicheActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        lectureSeule = !"visiteur".equals(sessionManager.getUserRole());
        ...
        if (lectureSeule) {
            btnAjouterHorsForfait.setVisibility(View.GONE);
            btnTransmettreFiche.setVisibility(View.GONE);
        }
    }
    ...
    private void chargerDetailFiche() {
        ...
        JsonObjectRequest request = new JsonObjectRequest(Request.Method.GET, url, null,
                response -> {
                    try {
                        if (!response.optBoolean("succes", false)) {
                            Toast.makeText(this, response.optString("message", getString(R.string.fiche_introuvable)), Toast.LENGTH_LONG).show();
                            return;
                        }
                        ...
                        textMontant.setText(getString(R.string.detail_montant, String.format(Locale.FRANCE, "%.2f", fiche.optDouble("montant_total"))));
                        textStatut.setText(getString(R.string.detail_statut, UiHelper.statutLisible(statut)));
                        textCommentaireVisiteur.setText(getString(R.string.detail_commentaire_visiteur, valeurOuDefaut(fiche.optString("commentaire_visiteur", ""))));
                        textCommentaireComptable.setText(getString(R.string.detail_commentaire_comptable, valeurOuDefaut(fiche.optString("commentaire_comptable", ""))));
                        ficheModifiable = !lectureSeule && UiHelper.estFicheModifiable(statut);
                ...
                error -> {
                    ...
                    Toast.makeText(this, ApiErrorHelper.extractMessage(error, getString(R.string.erreur_reseau_detail_fiche)), Toast.LENGTH_LONG).show();
                }) {
            @Override
    ...
    private void chargerHorsForfaits() {
        ...
        JsonObjectRequest request = new JsonObjectRequest(Request.Method.GET, url, null,
                    ...
                    try {
                        ...
                        if (items != null) {
                            for (int i = 0; i < items.length(); i++) {
                                ...
                                listeHorsForfaits.add(new HorsForfait(obj.optInt("id"), obj.optString("type_consommation"), obj.optString("date"), obj.optString("libelle"), obj.optDouble("montant"), obj.optString("commentaire")));
                            }
                        }
                        horsForfaitAdapter.notifyDataSetChanged();
                ...
                error -> {
                    textAucunHorsForfait.setVisibility(View.VISIBLE);
                    if (ApiErrorHelper.isAuthError(error)) { ApiErrorHelper.forceLogin(this, sessionManager, ApiErrorHelper.extractMessage(error, "Session expirée.")); return; }
                    Toast.makeText(this, ApiErrorHelper.extractMessage(error, getString(R.string.erreur_reseau_hors_forfaits)), Toast.LENGTH_LONG).show();
                }) {
            @Override
            public Map<String, String> getHeaders() throws AuthFailureError {
                Map<String, String> headers = new HashMap<>();
    ...
    private void transmettreFiche() {
        try {
            ...
            JsonObjectRequest request = new JsonObjectRequest(Request.Method.POST, ApiConfig.BASE_URL + "fiches.php", body,
                    response -> {
                        boolean succes = response.optBoolean("succes", false);
                        Toast.makeText(this, response.optString("message", getString(R.string.fiche_transmise_ok)), Toast.LENGTH_LONG).show();
                        if (succes) {
                    ...
                    error -> {
                        if (ApiErrorHelper.isAuthError(error)) { ApiErrorHelper.forceLogin(this, sessionManager, ApiErrorHelper.extractMessage(error, "Session expirée.")); return; }
                        Toast.makeText(this, ApiErrorHelper.extractMessage(error, getString(R.string.erreur_transmission_fiche)), Toast.LENGTH_LONG).show();
                    }) {
                @Override
                public Map<String, String> getHeaders() throws AuthFailureError {
                    Map<String, String> headers = new HashMap<>();
    ...
    private void appliquerEtatEdition() {
        if (lectureSeule) {
            textEtatEdition.setText(R.string.fiche_lecture_seule);
            return;
        }
    ...
    private String valeurOuDefaut(String valeur) {
        if (valeur == null || valeur.trim().isEmpty()) return getString(R.string.aucun_commentaire);
        return valeur;
    }
}
