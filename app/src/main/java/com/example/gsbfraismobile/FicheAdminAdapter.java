...
public class FicheAdminAdapter extends RecyclerView.Adapter<FicheAdminAdapter.ViewHolder> {
    ...
    @NonNull @Override public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        return new ViewHolder(LayoutInflater.from(context).inflate(R.layout.item_fiche_admin, parent, false));
    }
    ...
    private void demanderRefus(int idFiche) {
        ...
        new AlertDialog.Builder(context).setTitle(R.string.refuser_fiche).setView(input).setPositiveButton(R.string.refuser, (dialog, which) -> {
            ...
            if (commentaire.isEmpty()) { Toast.makeText(context, context.getString(R.string.commentaire_obligatoire), Toast.LENGTH_LONG).show(); return; }
    ...
    private void confirmerSuppression(int idFiche) {
        new AlertDialog.Builder(context).setTitle(R.string.supprimer_fiche_titre).setMessage(R.string.supprimer_fiche_message).setPositiveButton(R.string.supprimer, (dialog, which) -> envoyerAction(idFiche, "supprimer", "")).setNegativeButton(R.string.annuler, null).show();
    }

    private void envoyerAction(int idFiche, String action, String commentaire) {
        try {
            JSONObject body = new JSONObject();
            body.put("id_fiche", idFiche); body.put("action", action); body.put("commentaire", commentaire);
            JsonObjectRequest request = new JsonObjectRequest(Request.Method.POST, ApiConfig.BASE_URL + "admin_fiches.php", body,
                    ...
                    error -> {
                        ...
                        Toast.makeText(context, ApiErrorHelper.extractMessage(error, context.getString(R.string.erreur_reseau_admin_fiches)), Toast.LENGTH_LONG).show();
                    }) {
        ...
        } catch (Exception e) { Toast.makeText(context, context.getString(R.string.erreur_traitement), Toast.LENGTH_LONG).show(); }
    }

    static class ViewHolder extends RecyclerView.ViewHolder {
        TextView textNumero, textUtilisateur, textMois, textMontant, textStatut; Button btnValider, btnRefuser, btnSupprimer;
        ViewHolder(@NonNull View itemView) {
            super(itemView);
            textNumero = itemView.findViewById(R.id.textNumeroAdminFiche); textUtilisateur = itemView.findViewById(R.id.textUtilisateurAdminFiche); textMois = itemView.findViewById(R.id.textMoisAdminFiche); textMontant = itemView.findViewById(R.id.textMontantAdminFiche); textStatut = itemView.findViewById(R.id.textStatutAdminFiche); btnValider = itemView.findViewById(R.id.btnValiderAdmin); btnRefuser = itemView.findViewById(R.id.btnRefuserAdmin); btnSupprimer = itemView.findViewById(R.id.btnSupprimerFicheAdmin);
        }
    }
}
