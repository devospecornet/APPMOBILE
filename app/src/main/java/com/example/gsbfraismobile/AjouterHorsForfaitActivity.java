...
public class AjouterHorsForfaitActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (ficheId <= 0) { Toast.makeText(this, R.string.fiche_introuvable, Toast.LENGTH_LONG).show(); finish(); return; }
        ...
        btnEnregistrerHF.setOnClickListener(v -> {
            ...
            try {
                ...
                body.put("taux_tva", 20);

                JsonObjectRequest request = new JsonObjectRequest(Request.Method.POST, ApiConfig.BASE_URL + "hors_forfaits.php", body,
                        response -> {
                            boolean succes = response.optBoolean("succes", false);
                            Toast.makeText(this, response.optString("message", getString(R.string.hors_forfait_ajout_ok)), Toast.LENGTH_LONG).show();
                            if (succes) finish();
                        },
                        error -> {
                            ...
                            Toast.makeText(this, ApiErrorHelper.extractMessage(error, getString(R.string.erreur_ajout_hors_forfait)), Toast.LENGTH_LONG).show();
                        }) {
                    @Override
